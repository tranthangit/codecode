<?php /*a:1:{s:73:"/www/wwwroot/194.233.81.202/application/index/view/my/bind_trc20-red.html";i:1657445902;}*/ ?>
<!DOCTYPE html><!-- saved from url=(0041)http://qiang6-www.baomiche.com/#/BindCard --><html data-dpr="1" style="font-size: 37.5px;"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><meta name="viewport" content="width=device-width,user-scalable=no,initial-scale=1,maximum-scale=1,minimum-scale=1"><title>BINGK TRC20</title><link href="/static_new6/css/app.red.css" rel="stylesheet"><link rel="stylesheet" href="/static_new/css/public.css"><script charset="utf-8" src="/static_new/js/jquery.min.js"></script><script charset="utf-8" src="/static_new/js/dialog.min.js"></script><script charset="utf-8" src="/static_new/js/common.js"></script><style type="text/css" title="fading circle style">
        .circle-color-8 > div::before {
            background-color: #ccc;
        }
    </style></head><body style="font-size: 12px;"><div id="app"><div data-v-cc2885a2="" class="main"><div data-v-cc2885a2="" class="header"><div class="left_btn" onclick="window.history.back(-1)"><img
                    src="data:image/png;base64,"
                    alt="" class="return"></div><div class="Maintitle"><h3>BINGK TRC20</h3></div><div class="right_btn"></div></div><form action="" id="login-form"><div data-v-cc2885a2="" class="bink_card"><h3 data-v-cc2885a2=""><?php echo lang('您的身份信息'); ?></h3><ul data-v-cc2885a2="" class="bink_card_ul"><li data-v-cc2885a2=""><h4 data-v-cc2885a2=""><?php echo lang('真实姓名'); ?></h4><input data-v-cc2885a2="" type="text" name="username" maxlength="10" value="<?php echo htmlentities($info['username']); ?>" placeholder="<?php echo lang('未输入真实姓名,请设置'); ?>"></li><li data-v-cc2885a2=""><h4 data-v-cc2885a2=""><?php echo lang('联系电话'); ?></h4><input data-v-cc2885a2="" type="text" name="tel" maxlength="11" value="<?php echo htmlentities($info['tel']); ?>" placeholder="<?php echo lang('未输入电话,请设置'); ?>"></li></ul><h3 data-v-cc2885a2="">TRC20</h3><ul data-v-cc2885a2="" class="bink_card_ul"><li data-v-cc2885a2=""><h4 data-v-cc2885a2="" id="trc20"><?php echo lang('地址'); ?></h4><input id="cardnumtips" data-v-cc2885a2="" type="text" name="trc20" maxlength="34" value="<?php echo htmlentities($info['trc20']); ?>" placeholder="<?php echo lang('未输入地址,请设置'); ?>"></li></ul></div><button data-v-cc2885a2="" class="save-btn"><?php echo lang('设置信息'); ?></button></form><div data-v-cc2885a2="" class="agree" style="display: none;"><div data-v-cc2885a2="" class="agree_top"><input data-v-cc2885a2="" type="checkbox" id="agree"><label
                    data-v-cc2885a2="" for="agree"><?php echo lang('我已同意《服务协议》'); ?></label></div><div data-v-cc2885a2="" class="agree_bottom"><p data-v-cc2885a2=""><?php echo lang('尊敬的用户，为了保障您的资金安全，请您绑定您的真实姓名和设置取款密码。如果姓名与开户名不一致，将无法取款'); ?>。</p></div></div><button data-v-cc2885a2="" style="display: none;"><?php echo lang('确定绑卡'); ?></button></div></div><script type="application/javascript">    $(function () {
        /*检查表单*/
        function check() {
            if ($("input[name=username]").val() == '' || $("input[name=trc20]").val() == '') {
                $(document).dialog({infoText: '<?php echo lang('不能为空'); ?>！'});
                return false;
            }
            return true;
        }

        /*点击登录*/
        $(".save-btn").on('click', function () {
            if (check()) {
                var loading = null;
                $.ajax({
                    url: '/index/my/bind_trc20',
                    data: $("#login-form").serialize(),
                    type: 'POST',
                    beforeSend: function () {
                        loading = $(document).dialog({
                            type: 'notice',
                            infoIcon: '/static_new/img/loading.gif',
                            infoText: '<?php echo lang('正在加载中'); ?>',
                            autoClose: 0
                        });
                    },
                    success: function (data) {

                        if (data.code == 0) {
                            $(document).dialog({infoText: '<?php echo lang('保存成功'); ?>'});
                            setTimeout(function () {
                                window.history.back(-1);
                            }, 2000);
                        } else {
                            loading.close();
                            $(document).dialog({infoText: data.info});
                        }
                    }
                });
            }
            return false;
        })
    })

</script></body></html>