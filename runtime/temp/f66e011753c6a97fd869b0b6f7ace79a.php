<?php /*a:1:{s:81:"/www/wwwroot/194.233.81.202/application/index/view/ctrl/edit_deposit_pwd-red.html";i:1657445902;}*/ ?>
<!DOCTYPE html><!-- saved from url=(0053)http://qiang6-www.baomiche.com/#/PasswordDetailMobeil --><html data-dpr="1" style="font-size: 37.5px;"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><meta name="viewport" content="width=device-width,user-scalable=no,initial-scale=1,maximum-scale=1,minimum-scale=1"><title><?php echo lang('修改密码'); ?></title><link href="/static_new6/css/app.red.css" rel="stylesheet"><link rel="stylesheet" href="/static_new/css/public.css"><script charset="utf-8" src="/static_new/js/jquery.min.js"></script><script charset="utf-8" src="/static_new/js/dialog.min.js"></script><script charset="utf-8" src="/static_new/js/common.js"></script><style type="text/css" title="fading circle style">
        .circle-color-8 > div::before {
            background-color: #ccc;
        }
    </style></head><body style="font-size: 12px;"><div id="app"><div data-v-61ea9512="" class="main"><div data-v-61ea9512="" class="header"><div class="left_btn" onclick="window.history.back(-1)"><img
                        src="data:image/png;base64,"
                        alt="" class="return"></div><div class="Maintitle"><h3><?php echo lang('交易密码管理'); ?></h3></div><div class="right_btn"></div></div><form action="" id="forgetpwd-form"><div data-v-61ea9512="" class="bink_card" style="display: ;"><h3 data-v-61ea9512=""><?php echo lang('请输入您的新密码'); ?></h3><ul data-v-61ea9512="" class="bink_card_ul"><li data-v-61ea9512=""><h4 data-v-61ea9512=""><?php echo lang('登录密码'); ?></h4><input data-v-61ea9512="" type="text" name="old_pwd" placeholder="<?php echo lang('请输入登录密码'); ?>"><input data-v-61ea9512="" type="hidden" name="type" value="2"></li><li data-v-61ea9512=""><h4 data-v-61ea9512=""><?php echo lang('新密码'); ?></h4><input data-v-61ea9512="" name="new_pwd"  type="text" placeholder="<?php echo lang('请输入新密码'); ?>"></li><li data-v-61ea9512=""><h4 data-v-61ea9512=""><?php echo lang('确认密码'); ?></h4><input data-v-61ea9512="" name="password_confirm" type="text" placeholder="<?php echo lang('请再次输入密码'); ?>"></li></ul><p data-v-cc2885a2="" style="padding: 20px"><?php echo lang('请牢记密码,如忘记密码,请联系客服'); ?>.</p><!--            <h3 data-v-61ea9512=""><?php echo lang('请输入您的提现密码'); ?></h3>--><!--            <ul data-v-61ea9512="" class="bink_card_ul Cash_num_password">--><!--                <li data-v-61ea9512=""><h4 data-v-61ea9512=""><?php echo lang('提现密码'); ?></h4>--><!--                    <input data-v-61ea9512="" type="tel" maxlength="1" class="border-input">--><!--                    <input data-v-61ea9512="" type="tel" maxlength="1" class="border-input">--><!--                    <input data-v-61ea9512="" type="tel" maxlength="1" class="border-input">--><!--                    <input data-v-61ea9512="" type="tel" maxlength="1" class="border-input">--><!--                </li>--><!--            </ul>--></div><button data-v-61ea9512="" class="form-buttom"><?php echo lang('下一步'); ?></button><button data-v-61ea9512="" style="display: none;"><?php echo lang('确定修改'); ?></button></form></div></div><script type="application/javascript">
    $(function () {
        var countdown = 120;
        var flag = true;
        var loading = null;

        function check() {
            if ($("input[name=old_pwd]").val() == '' || $("input[name=new_pwd]").val() == '') {
                $(document).dialog({infoText: '<?php echo lang('请输入密码'); ?>'});
                return false;
            }
            if ($("input[name=new_pwd]").val() !== $("input[name=password_confirm]").val()) {
                $(document).dialog({infoText: '<?php echo lang('两次输入的密码不一致'); ?>'});
                return false;
            }
            return true;
        }

        $(".form-buttom").on('click', function () {
            if (check()) {
                $.ajax({
                    url: '/index/ctrl/set_pwd',
                    data: $("#forgetpwd-form").serialize(),
                    type: 'POST',
                    beforeSend: function () {
                        loading = $(document).dialog({
                            type: 'notice',
                            infoIcon: '/static_new/img/loading.gif',
                            infoText: '<?php echo lang('正在加载中'); ?>',
                            autoClose: 0
                        });
                    },
                    success: function (data) {
                        if (data.code==0) {
                            $(document).dialog({infoText: data.info});
                            setTimeout(function () {
                                window.location.href = '/index/ctrl/set';
                            }, 2000);
                        } else {
                            loading.close();
                            $(document).dialog({infoText: data.info});
                        }
                    }
                });
            }
            return false;
        })
    })
</script></body></html>