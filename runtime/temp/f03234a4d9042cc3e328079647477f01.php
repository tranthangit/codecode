<?php /*a:1:{s:76:"/www/wwwroot/194.233.81.202/application/index/view/my/edit_username-red.html";i:1657445902;}*/ ?>
<!DOCTYPE html><!-- saved from url=(0053)http://qiang6-www.baomiche.com/#/PasswordDetailMobeil --><html data-dpr="1" style="font-size: 37.5px;"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"><meta name="viewport" content="width=device-width,user-scalable=no,initial-scale=1,maximum-scale=1,minimum-scale=1"><title><?php echo lang('修改密码'); ?></title><link href="/static_new6/css/app.red.css" rel="stylesheet"><link rel="stylesheet" href="/static_new/css/public.css"><script charset="utf-8" src="/static_new/js/jquery.min.js"></script><script charset="utf-8" src="/static_new/js/common.js"></script><script charset="utf-8" src="/static_new/js/dialog.min.js"></script><style type="text/css" title="fading circle style">
        .circle-color-8 > div::before {
            background-color: #ccc;
        }
    </style></head><body style="font-size: 12px;"><div id="app"><div data-v-61ea9512="" class="main"><div data-v-61ea9512="" class="header"><div class="left_btn" onclick="window.history.back(-1)"><img
                        src="data:image/png;base64,"
                        alt="" class="return"></div><div class="Maintitle"><h3><?php echo lang('修改名称'); ?></h3></div><div class="right_btn"></div></div><div data-v-61ea9512="" class="bink_card"><h3 data-v-61ea9512=""><?php echo lang('请修改您的名称'); ?></h3><ul data-v-61ea9512="" class="bink_card_ul"><li data-v-61ea9512=""><h4 data-v-61ea9512=""><?php echo lang('名称'); ?></h4><input data-v-61ea9512="" type="text" placeholder="<?php echo lang('输入名称'); ?>" value="<?php echo htmlentities($info['username']); ?>" name="username" class="code"></li></ul></div><button data-v-61ea9512="" class="save-btn"><?php echo lang('确定修改'); ?></button></div></div><script>
    $(function () {
        function check() {
            if ($("input[name=username]").val() == '') {
                $(document).dialog({infoText: '<?php echo lang('用户名不能为空'); ?>！'});
                return false;
            }
            return true;
        }

        /*点击登录*/
        $(".save-btn").on('click', function () {
            if (check()) {
                var loading = null;
                $.ajax({
                    url: "",
                    data: {username:$("input[name=username]").val()},
                    type: 'POST',
                    beforeSend: function () {
                        loading = $(document).dialog({
                            type: 'notice',
                            infoIcon: '/static_new/img/loading.gif',
                            infoText: '<?php echo lang('正在加载中'); ?>',
                            autoClose: 0
                        });
                    },
                    success: function (data) {

                        if (data.code == 0) {
                            $(document).dialog({infoText: '<?php echo lang('保存成功'); ?>'});
                            setTimeout(function () {
                                window.history.back(-1);
                            }, 2000);
                        } else {
                            loading.close();
                            $(document).dialog({infoText: data.info});
                        }
                    }
                });
            }
        });
    })
</script></body></html>